DateTime 比较 该示例选取中国标准时间（CST，时区 ID 为“Asia/Shanghai”）和美国东部夏令时时间（EDT，时区 ID 为“America/New_York”）进行时间比较。
>>>>
>>>>> **说明：** 示例中使用 [TimeZone.load](../time_package_api/time_package_classes.html#static-func-loadstring) 函数加载时区信息，在不同平台上加载时区信息有不同的依赖，用户需按其要求进行设置。
>>>>     
>>>>     import std.time.*
>>>>     
>>>>     main() {
