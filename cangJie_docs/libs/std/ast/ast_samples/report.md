自定义报错接口 仓颉 ast 包提供了自定义报错接口 [diagReport](../ast_package_api/ast_package_funcs.html#func-diagreportdiagreportlevel-tokens-string-string)。方便定义宏的用户，在解析传入 [Tokens](../ast_package_api/ast_package_classes.html#class-tokens) 时，对错误 [Tokens](../ast_package_api/ast_package_classes.html#class-tokens) 内容进行自定义报错。 自定义报错接口提供同原生编译器报错一样的输出格式，允许用户报 `WARNING` 和 `ERROR` 两类错误提示信息。 正确使用示例 宏定义如下：
>     
>     // macro_definition.cj
