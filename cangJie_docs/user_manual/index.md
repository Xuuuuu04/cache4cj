...
**值得注意的是** ，若编译可执行程序时链接了仓颉的动态库文件，必须同时指定 `\--dy-std` 与 `\--dy-libs` 选项，详情请见 `\--dy-std` 选项说明。
...
`cjc` 支持以下宏选项，关于宏的更多内容请参见[“宏”](./Chapter_15_Macro.html)章节。
...
`cjc` 支持以下条件编译选项，关于条件编译的更多内容请参见[“条件编译”](../compile_and_build/conditional_compilation.html)。
