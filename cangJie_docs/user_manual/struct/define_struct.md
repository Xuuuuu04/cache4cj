...
`struct` 类型的定义以关键字 `struct` 开头，后跟 `struct` 的名字，接着是定义在一对花括号中的 `struct` 定义体。`struct` 定义体中可以定义一系列的成员变量、成员属性（参见[属性](../class_and_interface/prop.html)）、静态初始化器、构造函数和成员函数。
...
一个 `struct` 中可以定义多个普通构造函数，但它们必须构成重载（参见[函数重载](../function/function_overloading.html)），否则报重定义错误。
...
`struct` 的成员（包括成员变量、成员属性、构造函数、成员函数、操作符函数（详见[操作符重载](../function/operator_overloading.html)章节））用 4 种访问修饰符修饰：`private`、`internal`、`protected` 和 `public`，缺省的修饰符是 `internal`。

        * `private` 表示在 `struct` 定义内可见。
        * `internal` 表示仅当前包及子包（包括子包的子包，详见[包](../package/toplevel_access.html)章节）内可见。
        * `protected` 表示当前模块（详见[包](../package/toplevel_access.html)章节）可见。
        * `public` 表示模块内外均可见。
